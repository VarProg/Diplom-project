...
@section('users')
        <main id="js-page-content" role="main" class="page-content mt-3">
            ...
            <div class="row" id="js-contacts">
                <div class="col-xl-4">
                    <div id="c_1" class="card border shadow-0 mb-g shadow-sm-hover" data-filter-tags="{{Auth::user()->info->name}}">
                        <div class="card-body border-faded border-top-0 border-left-0 border-right-0 rounded-top">
                            <div class="d-flex flex-row align-items-center">
                            @switch(Auth::user()->info->status ?? '')
                                @case('online')
                            ...
                            @endswitch
                                    ...
                                    <a href="/page_profile/{{Auth::user()->id}}"><span class="rounded-circle profile-image d-block " style="background-image:url('/{{Auth::user()->info->image ?? "img/demo/avatars/avatar-m.png"}}'); background-size: cover;"></span></a>
                                </span>
                                <div class="info-card-text flex-1">
                                    ...
                                    <div class="dropdown-menu">
                                        <a class="dropdown-item" href="/page_edit/{{Auth::user()->id}}">
                                            <i class="fa fa-edit"></i>
                                        Редактировать</a>
                                        <a class="dropdown-item" href="/page_security/{{Auth::user()->id}}">
                                            <i class="fa fa-lock"></i>
                                        Безопасность</a>
                                        <a class="dropdown-item" href="/page_status/{{Auth::user()->id}}">
                                            <i class="fa fa-sun"></i>
                                        Установить статус</a>
                                        <a class="dropdown-item" href="/page_media/{{Auth::user()->id}}">
                                            <i class="fa fa-camera"></i>
                                            Загрузить аватар
                                        </a>
                                        <a href="/delete_user/{{Auth::user()->id}}" class="dropdown-item" onclick="return confirm('are you sure?');">
                                            <i class="fa fa-window-close"></i>
                                            Удалить
                                        </a>
                                    </div>
                                    <span class="text-truncate text-truncate-xl">{{Auth::user()->info->job ?? ''}}</span>
                                </div>
                        ...
                        <div class="card-body p-0 collapse show">
                            <div class="p-3">
                                <a href="#" class="mt-1 d-block fs-sm fw-400 text-dark">
                                    <i class="fas fa-mobile-alt text-muted mr-2"></i>{{Auth::user()->info->phone ?? ''}}</a>
                                <a href="#" class="mt-1 d-block fs-sm fw-400 text-dark">
                                    <i class="fas fa-mouse-pointer text-muted mr-2"></i>{{Auth::user()->email}}</a>
                                <address class="fs-sm fw-400 mt-4 text-muted">
                                    <i class="fas fa-map-pin mr-2"></i>{{Auth::user()->info->adress ?? ''}}</address>
                ...
                {{-- Блок со всеми остальными пользователями без возможности редактирования или если зашел как админ--}}
